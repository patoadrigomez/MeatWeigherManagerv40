...
{
    class CABM_DestinosDlg : CABM_DB
    {
        #region Privates VARIABLES AND PROPERTIES 
        private TextBox textBox_nombre;
        private Label label1;
        CDestino m_destinoSelected;
        private BEHAVIOR_MODE m_mode;
        #endregion

        #region Public VARIABLES AND PROPERTIES 
        public CDestino DestinoSelected { get => m_destinoSelected; set => m_destinoSelected = value; }
        ...
        public CABM_DestinosDlg(OleDbConnection oleDbConnection,BEHAVIOR_MODE mode =BEHAVIOR_MODE.ABM, int idSelection = 0) : base(oleDbConnection, idSelection)
        {
            InitializeComponent();
            m_mode = mode;
            base.Text = "MeatWeigherManager - Mantenimiento de Tabla de Destinos";
            NameTable = "Destinos";
            NameFieldID = "ID";
            NameFieldPrincipal = "Nombre";
            NameFieldFilterDataGrid = "Nombre";
            NameRegister = "Destino";
            LabelDataGrid = NameRegister;
            LabelTextBoxFilterDataGrid = "Busqueda por "+ NameFieldFilterDataGrid;
            SqlQuery_LoadDataGrid = "SELECT NOMBRE,ID FROM Destinos WHERE 1=1 ";
            OnDGV_CellDoubleClick += CABM_DestinosDlg_OnDGV_CellDoubleClick;
            OnDGV_SelectionChanged += CABM_DestinosDlg_OnDGV_SelectionChanged;

            DestinoSelected = new CDestino();
            if (m_mode == BEHAVIOR_MODE.SELECTION)
            {
                base.Text = "MeatWeigherManager - SELECCIÓN DE DESTINO";
        ...
        private void CABM_DestinosDlg_OnDGV_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
        ...
        private void CABM_DestinosDlg_OnDGV_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
        }
        #endregion
