...
{
    class CABM_SectoresDlg : CABM_DB
    {
        #region Privates VARIABLES AND PROPERTIES 
        private TextBox textBox_nombre;
        private Label label1;
        CSector m_sectorSelected;
        private BEHAVIOR_MODE m_mode;
        #endregion

        #region Public VARIABLES AND PROPERTIES 
        public CSector SectorSelected { get => m_sectorSelected; set => m_sectorSelected = value; }
        ...
        public CABM_SectoresDlg(OleDbConnection oleDbConnection,BEHAVIOR_MODE mode =BEHAVIOR_MODE.ABM, int idSelection = 0) : base(oleDbConnection, idSelection)
        {
            InitializeComponent();
            m_mode = mode;
            base.Text = "MeatWeigherManager - Mantenimiento de Tabla de Sectores";
            NameTable = "Sectores";
            NameFieldID = "ID";
            ...
            SqlQuery_LoadDataGrid = "SELECT NOMBRE,ID FROM Sectores WHERE 1=1 ";
            OnDGV_CellDoubleClick += CABM_SectoresDlg_OnDGV_CellDoubleClick;
            OnDGV_SelectionChanged += CABM_SectoresDlg_OnDGV_SelectionChanged;

            SectorSelected = new CSector();
            if (m_mode == BEHAVIOR_MODE.SELECTION)
            {
                base.Text = "MeatWeigherManager - SELECCIÓN DE SECTOR";
        ...
        private void CABM_SectoresDlg_OnDGV_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
        ...
        private void CABM_SectoresDlg_OnDGV_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
        }
        #endregion
